...
| 状态 | 含义 | 超时与状态迁移路径 |
|:---|:---|:---|
| **REACHABLE** | 已确认可达（最近收到过来自该邻居的确认，如TCP ACK） | 默认基准值`base_reachable_time`为30秒，**实际超时在15~45秒随机抖动**，若无确认则迁移至STALE |
...
> **关键认知**：`gc_stale_time`控制**内核垃圾回收（GC）的扫描间隔**，而非单个ARP条目的老化时间。**条目的状态迁移由`base_reachable_time`与`reachable_time`控制**。主动删除发生在PROBE失败后。实际GC回收受`gc_thresh1/2/3`（缓存表大小阈值）控制。
...
**【修正】Linux `arp_accept` 机制澄清**：

`arp_accept`控制当内核收到**未经请求的ARP报文（Gratuitous ARP）** 时，是否在本地ARP缓存中**新建或更新**IP-MAC条目。

**该参数的默认值自Linux内核版本 2.6.39 / 3.0 起已由 0 变更为 1**。这意味着运行现代主流内核（包括但不限于CentOS 7、Ubuntu 16.04及以上）的Linux主机，**默认均会接受未经请求的ARP报文并更新缓存**。

在实际渗透测试场景中，`arp_accept=1`的默认配置使得ARP欺骗攻击更容易成功。**防御方不应依赖终端侧的`arp_accept`参数，而应依靠DAI进行二层防护**。

### 5.2 Windows ARP缓存

- Windows动态调整缓存生命周期，**条目数 < 100时，默认约2分钟**；**条目数 > 100时，约10分钟**。
- 可通过注册表`HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters`调整（**修改后需重启生效**）：
...
2. **无条件覆盖**：**当前主流操作系统（包括Windows 10/11、Linux内核2.6.39+、macOS）默认均接受未经请求的ARP Reply并更新本地ARP缓存**，这是ARP欺骗能够成功的基础。
...
! 2. 上联端口（连接路由器/核心）设为 DHCP Snooping Trust
interface GigabitEthernet0/1
 description Uplink_to_Core
 ip dhcp snooping trust
 ! ★ 注意：DAI信任应谨慎配置 —— 仅当该端口为纯二层Trunk且连接绝对可信路由器时才启用
 ! 若上联为三层路由口或SVI，此命令无效。更佳实践：仅在接入端口做DAI检查，
 ! 上联口通过全局 ip arp inspection vlan 10,20 自动应用，不额外配置信任（保持非信任）
 ! 以下命令根据实际拓扑评估，非必须：
 ! ip arp inspection trust

! 3. 接入端口（连接终端）—— 默认非信任，无需显式配置
interface GigabitEthernet0/2
 description Access_to_PC
 ip dhcp snooping limit rate 10   ! DHCP请求速率限制（防DHCP饥饿）
 ! 注意：此端口不配置 ip arp inspection trust，保持默认非信任状态

! 4. 全局开启DAI（针对VLAN）
ip arp inspection vlan 10,20

! 5. 【可选】对静态IP服务器配置ARP ACL（端口保持非信任）
arp access-list STATIC_SERVER_ARP
 permit ip host 10.0.20.10 mac host 0050.56a1.b2c3
 deny ip any mac any        ! 显式拒绝其他IP-MAC组合，增强安全性
!
interface GigabitEthernet0/3
 description Static_Server
 ip arp inspection filter STATIC_SERVER_ARP vlan 20
 ! 端口保持非信任（不配置 ip arp inspection trust），确保DAI生效
```

> **⚠️ 静态IP避坑（关键）** ：对于配置静态IP的服务器（因其不走DHCP，绑定表无记录），DAI会丢弃其正常ARP包。解决方案：使用上述ARP ACL显式放行。
>
> **三层交换机SVI特殊说明**：若网关SVI位于本机，**切勿在SVI接口下配置`ip arp inspection trust`**（默认非信任），否则DAI将对所有本机应答的ARP报文失效，导致防御形同虚设。

**性能考量与排障**：
- DAI启用后，交换机需拦截每个ARP包并查询绑定表，建议在**上联端口谨慎配置信任**以降低CPU负载（但需确保物理安全）；
...
**【修正】攻击者利用Proxy ARP进行欺骗的典型场景**：
① 主机配置了**宽松的子网掩码**（误将跨网段IP视为同网段），导致本应发往网关的流量直接发往目标IP，此时攻击者可应答该IP的ARP请求；
② 在**多接口主机（如VPN网关）**上，若`arp_filter`等参数配置不当，攻击者可诱导主机响应非本接口的ARP请求。

**但需强调的是**：在生产网络中，**禁用代理ARP（`no ip proxy-arp`）仅能防御上述场景，无法防御基于虚假应答的ARP欺骗**。真正的核心防御始终为第9.2节的**DAI**。
...
**增强检测**：配合Elasticsearch聚合分析，在5分钟内检测同一MAC对应超过3个IP的异常事件，可大幅提升检测精度。
...
**此过程实现了跨VLAN MITM，且完全不需要依赖代理ARP**。代理ARP仅在主机子网掩码配置错误或特殊路由场景下才起作用，并非跨VLAN攻击的必要条件。
